...
func (c *Config) CreateUsersDB() (*gorm.DB, error) {
	...
	if c.DatabaseConnectionURI != "" {
		dbDSN = c.DatabaseConnectionURI
	} else if c.postgresUsersDB == "" {
...
func Load() *Config {
	...
	databaseConnectionURI := os.Getenv(config_env.DATABASE_CONNECTION_URI)

	if databaseConnectionURI == "" && postgresUsersDB == "" && (postgresHost == "" || postgresPort == "" || postgresUser == "" || postgresPassword == "" || postgresDB == "") {
		logger.LogWarn("[CONFIG] PostgreSQL variables/URI missing. Checking for SQLite...")
		if !strings.Contains(postgresUsersDB, ".db") && !strings.Contains(os.Getenv("POSTGRES_AUTH_DB"), ".db") {
	...
	databaseAutoMigrate := os.Getenv(config_env.DATABASE_AUTO_MIGRATE)
	...
	config := &Config{
		...
		DatabaseAutoMigrate:  databaseAutoMigrate != "false", // Default true
