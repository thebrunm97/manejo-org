...
func (s *pollService) autoMigrate() error {
	...
	if err != nil {
		// Error SQLSTATE 42P07 in Postgres (relation already exists)
		if strings.Contains(err.Error(), "already exists") || strings.Contains(err.Error(), "42P07") {
			s.loggerWrapper.GetLogger("poll-service").LogInfo("[POLL] Table poll_votes already exists, skipping creation.")
			return nil
		}
