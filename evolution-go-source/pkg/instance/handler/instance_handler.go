...
func (h *instanceHandler) SetWebhook(c *gin.Context) {
	...
	var data WebhookStruct
	if err := c.ShouldBindJSON(&data); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}

	err := h.instanceService.SetWebhook(instanceName, data.Url, data.Enabled, data.Events)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}

	c.JSON(http.StatusOK, gin.H{"message": "success", "webhook": data.Url})
}
