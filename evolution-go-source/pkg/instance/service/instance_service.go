...
type InstanceService interface {
	...
	SetWebhook(instanceName string, url string, enabled bool, events []string) error
}
...
// Reconnect força uma reconexão limpa da instância.
//
// DT-53 (pmo-bot-go): antes, esta função chamava ensureClientConnected primeiro,
// que devolve erro "client disconnected" exatamente quando o cliente existe no
// pool mas o socket caiu — o caso mais comum de queda, e o que a rota
// /instance/reconnect deveria resolver. Como o erro retornava antes de
// ReconnectClient ser chamado, a rota só "funcionava" quando a sessão já
// estava conectada (não fazia nada) ou quando não havia cliente algum no pool.
// ReconnectClient (whatsmeow.go) já trata todos os estados sozinho — desliga
// o cliente existente se houver, limpa os recursos e sobe um novo —, então o
// gatekeeper nunca foi necessário aqui.
...
func (i instances) GetInstanceByToken(token string) (*instance_model.Instance, error) {
	inst, err := i.instanceRepository.GetInstanceByToken(token)
	if err != nil && token == i.config.GlobalApiKey {
		// Tenta buscar pelo nome manejo-org
		instByName, errName := i.instanceRepository.GetInstanceByName("manejo-org")
		if errName == nil && instByName != nil {
			instByName.Token = token
			_ = i.instanceRepository.Update(instByName)
			return instByName, nil
		}
		// Auto-cria a instância manejo-org
		createStruct := &CreateStruct{
			InstanceId: "manejo-org",
			Name:       "manejo-org",
			Token:      token,
		}
		return i.Create(createStruct)
	}
	return inst, err
}
...
func (i instances) SetWebhook(instanceName string, url string, enabled bool, events []string) error {
	instance, err := i.instanceRepository.GetInstanceByName(instanceName)
	if err != nil {
	...
	if !enabled || url == "" {
		...
		var subscribedEvents []string
		for _, arg := range events {
			if arg == "MESSAGES_UPSERT" {
				arg = event_types.MESSAGE
			}
			subscribedEvents = append(subscribedEvents, arg)
		}
	...
	_ = i.whatsmeowService.UpdateInstanceSettings(instance.Id)

	i.loggerWrapper.GetLogger(instance.Id).LogInfo("[%s] Webhook updated successfully: %s", instance.Id, url)
	return nil
}
