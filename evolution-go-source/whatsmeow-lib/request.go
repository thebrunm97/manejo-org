...
func (cli *Client) receiveResponse(ctx context.Context, data *waBinary.Node) bool {
	...
	if !ok {
		...
		cli.Log.Debugf("receiveResponse: no waiter found for id=%s, tag=%s", id, data.Tag)
		return false
	}
	...
	cli.Log.Debugf("receiveResponse: found waiter for id=%s, tag=%s, sending response", id, data.Tag)

	select {
	case waiter <- data:
		cli.Log.Debugf("receiveResponse: response sent successfully for id=%s", id)
	case <-ctx.Done():
...
func (cli *Client) sendIQAsyncAndGetData(ctx context.Context, query *infoQuery) (<-chan *waBinary.Node, []byte, error) {
	...
	cli.Log.Debugf("sendIQAsyncAndGetData: sending IQ, id=%s, namespace=%s, to=%s, target=%s", query.ID, query.Namespace, attrs["to"], attrs["target"])
	...
	cli.Log.Debugf("sendIQAsyncAndGetData: IQ sent successfully, id=%s", query.ID)
...
func (cli *Client) sendIQ(ctx context.Context, query infoQuery) (*waBinary.Node, error) {
	...
	cli.Log.Debugf("sendIQ: namespace=%s, id=%s, timeout=%v", query.Namespace, query.ID, query.Timeout)
	...
	cli.Log.Debugf("sendIQ: waiting for response, id=%s", query.ID)

	select {
	case res := <-resChan:
		cli.Log.Debugf("sendIQ: received response, id=%s, tag=%s", query.ID, res.Tag)
