...
export const locationService = {
   ...
   */
  createCanteiro: async (
    talhaoId: number,
    nome: string,
    metadata: any = {},
  ): Promise<CanteiroRow> => {
    const payload: CanteiroInsert & { geometry?: any } = {
      talhao_id: talhaoId,
      nome: nome,
   ...
   */
  createCanteirosBatch: async (
    payloads: CanteiroInsert[],
  ): Promise<CanteiroRow[]> => {
    const { data, error } = await supabase
      .from("canteiros")
      .insert(payloads)
  ...
  updateTalhao: async (
    id: number,
    data: Partial<TalhaoRow>,
  ): Promise<boolean> => {
    const { error } = await supabase.from("talhoes").update(data).eq("id", id);
   ...
   */
  updateCanteiro: async (
    id: string,
    data: any,
  ): Promise<boolean> => {
    const { error } = await supabase.from("canteiros").update(data).eq("id", id);
