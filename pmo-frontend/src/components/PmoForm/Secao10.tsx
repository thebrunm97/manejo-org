    ...
    <div className="rounded-lg border-l-[6px] border-l-amber-400 border border-gray-200 bg-white mb-2 shadow-sm">
      <div className="p-3">
        <div className="flex justify-between items-start">
          ...
          <div className="flex">
            <button type="button" onClick={onEdit} className="p-1.5 text-blue-500 hover:bg-blue-50 rounded"><Pencil size={16} /></button>
            <button type="button" onClick={onDelete} className="p-1.5 text-red-500 hover:bg-red-50 rounded"><Trash2 size={16} /></button>
          </div>
        </div>
        ...
        <div className="mt-2">
          <button type="button" onClick={() => setExp(!exp)} className="inline-flex items-center gap-1 text-xs text-gray-500 hover:text-gray-700">{exp ? 'Ocultar' : 'Ver detalhes'}{exp ? <ChevronUp size={14} /> : <ChevronDown size={14} />}</button>
          {exp && <div className="mt-2 p-3 bg-gray-50 rounded-md text-sm space-y-1.5">{item.quando && <div><strong>Quando / Frequência:</strong> {item.quando}</div>}{item.procedencia && <div><strong>Procedência:</strong> {item.procedencia}</div>}{item.composicao && <div><strong>Composição:</strong> {item.composicao}</div>}{item.marca && <div><strong>Marca:</strong> {item.marca}</div>}</div>}
        </div>
      </div>
    </div>
  );
    ...
    <>
      <SectionContainer title="Manejo Fitossanitário (Pragas e Doenças)" onAdd={addNew} addButtonLabel="Adicionar Manejo" isEmpty={items.length === 0} emptyMessage="Nenhum manejo registrado." icon={<Bug size={48} className="text-gray-400" />} sectionId={10} onApplySuggestion={applySug}>
        <div className="flex flex-col">
          ...
          <button type="button" onClick={addNew} className="inline-flex items-center gap-1.5 mt-2 px-3 py-1.5 text-sm text-amber-700 border border-dashed border-amber-300 hover:bg-amber-50 rounded-md self-start font-semibold"><Plus size={16} />Adicionar Outro</button>
        </div>
      </SectionContainer>
      ...
      {mOpen && (<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4"><div className="bg-white rounded-xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-200"><h3 className="text-lg font-bold text-gray-800">{ei?.produto_ou_manejo ? 'Editar Manejo' : 'Novo Manejo'}</h3><button type="button" onClick={() => setMOpen(false)} className="p-1 hover:bg-gray-100 rounded"><X size={20} /></button></div>
        <div className="p-5"><p className="text-sm text-gray-500 mb-4">Preencha as informações sobre o produto ou técnica.</p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="sm:col-span-2"><label className={lCls}>Alvo Principal</label><input className={iCls} value={ei?.alvo_principal || ''} onChange={e => fc('alvo_principal', e.target.value)} placeholder="Ex: Formigas, Ferrugem..." /></div>
            ...
            <div><label className={lCls}>Quando / Frequência?</label><input className={iCls} value={ei?.quando || ''} onChange={e => fc('quando', e.target.value)} placeholder="Ex: No plantio, a cada 15 dias..." /></div>
        ...
        <div className="flex justify-end gap-2 px-5 py-4 border-t border-gray-200"><button type="button" onClick={() => setMOpen(false)} className="px-4 py-2 text-sm text-gray-600 hover:bg-gray-100 rounded-md">Cancelar</button><button type="button" onClick={saveModal} className="px-4 py-2 text-sm bg-amber-500 text-white rounded-md hover:bg-amber-600 font-medium">Salvar</button></div>
      </div></div>)}
      ...
      {dOpen && (<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4"><div className="bg-white rounded-xl shadow-xl w-full max-w-sm">
        ...
        <div className="flex justify-end gap-2 px-5 py-4 border-t border-gray-200"><button type="button" onClick={() => { setDOpen(false); setDId(null) }} className="px-4 py-2 text-sm text-gray-600 hover:bg-gray-100 rounded-md">Cancelar</button><button type="button" onClick={confirmDel} className="px-4 py-2 text-sm bg-red-600 text-white rounded-md hover:bg-red-700 font-medium">Excluir</button></div>
      </div></div>)}
    </>
  );
};
