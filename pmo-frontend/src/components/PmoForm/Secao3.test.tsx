...
describe('Secao3', () => {
    test('deve atualizar o radio de produtos_nao_certificados', () => {
        ...
        render(<Secao3 data={data} onSectionChange={mockOnSectionChange} />);
    ...
    test('deve renderizar tabelas apenas quando Sim é selecionado', () => {
        ...
        const { rerender } = render(<Secao3 data={{ produtos_nao_certificados: false }} onSectionChange={mockOnSectionChange} />);
        expect(screen.queryByText('3.1. Produção Primária Vegetal Não Orgânica')).not.toBeInTheDocument();

        rerender(<Secao3 data={{ produtos_nao_certificados: true }} onSectionChange={mockOnSectionChange} />);
