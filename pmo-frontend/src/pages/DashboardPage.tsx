        ...
        <div className="text-right">
          ...
          <div className="flex items-center gap-1 justify-end">
            <CloudRain
              size={16}
              className={
                rainChance && rainChance > 50 ? "text-sky-600" : "text-cyan-400"
              }
            />
...
const DashboardPage: React.FC = () => {
  ...
  return (
    <div className="pb-8 overflow-x-hidden">
      ...
      {isLoading ? (
        /* ---------------- SKELETON LOADER ---------------- */
        <div className="flex flex-col lg:flex-row gap-6 items-start mb-8 w-full min-w-0 animate-pulse">
          {/* Left Column Skeleton */}
          <div className="flex flex-col gap-6 w-full lg:w-1/3 min-w-0">
            {/* Plano Atual Skeleton */}
            <div className="h-[220px] bg-white rounded-3xl border border-slate-100 shadow-sm p-6 w-full">
            ...
            {/* Weather Skeleton */}
            <div className="h-[160px] bg-white rounded-3xl border border-slate-100 shadow-sm p-6 w-full">
            ...
            {/* Bot Skeleton */}
            <div className="h-[200px] bg-white rounded-3xl border border-slate-100 shadow-sm p-6 w-full">
              <div className="bg-slate-300 w-10 h-10 rounded-lg mb-4"></div>
              <div className="w-2/3 h-5 bg-slate-300 rounded mb-6"></div>
              <div className="w-full h-10 bg-slate-300 rounded"></div>
            </div>
          </div>

          {/* Right Column Skeleton (Harvest Stats) */}
          <div className="bg-white rounded-3xl border border-slate-100 p-6 shadow-sm min-h-[600px] w-full lg:w-2/3 min-w-0 flex flex-col">
            <div className="w-1/3 h-6 bg-slate-300 rounded mb-8"></div>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-8">
              <div className="h-24 bg-slate-300 rounded-xl"></div>
              <div className="h-24 bg-slate-300 rounded-xl"></div>
              <div className="h-24 hidden md:block bg-slate-300 rounded-xl"></div>
            </div>
            <div className="w-1/4 h-5 bg-slate-300 rounded mb-6 mt-8"></div>
            <div className="space-y-4">
              <div className="w-full h-16 bg-slate-300 rounded-xl"></div>
              <div className="w-full h-16 bg-slate-300 rounded-xl"></div>
              <div className="w-full h-16 bg-slate-300 rounded-xl"></div>
              <div className="w-full h-16 bg-slate-300 rounded-xl"></div>
            </div>
          ...
          <div className="flex flex-col gap-6 w-full lg:w-1/3 min-w-0">
            ...
            <div id="tour-whatsapp-card" className="h-full w-full">
              <div className="bg-white rounded-3xl border border-slate-200 p-6 shadow-sm hover:shadow-md transition-shadow h-full">
                <div className="flex justify-between items-start mb-2">
                  <div
                    className={`p-2 rounded-lg ${userProfile?.telefone ? "bg-green-100/50 text-green-600" : "bg-slate-100 text-slate-500"}`}
                  >
                    <Smartphone size={20} />
                  </div>
                  <span
                    className={`px-2 py-1 rounded-md text-[10px] font-bold uppercase ${userProfile?.telefone ? "bg-green-100 text-green-700" : "bg-slate-100 text-slate-500"}`}
                  >
