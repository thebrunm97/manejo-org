...
- **Extração e Salvamento N:N de Canteiros**:
  - `internal/groq/client.go`: `Localizacao.Canteiros` alterado para `[]string` e prompt atualizado para extração de array JSON.
  - `internal/supabase/client.go`: `InsertCadernoCampo` agora retorna UUID via `return=representation`.
  - `internal/supabase/client.go`: Novos métodos `LookupCanteiroIDs` (com filtro numérico fino) e `InsertCanteiroVinculos` (junction table).
  - `internal/state/fsm.go`: Orquestração do salvamento relacional com fallback silencioso e mensagem de sucesso enriquecida.
- **Dashboard Logs & FSM Fix**:
  - `internal/groq/client.go`: Captura de `TokensPrompt` e `TokensCompletion` de `chatResp.Usage`.
  - `internal/supabase/client.go`: Nova struct `LogProcessamentoInsert` e método `InsertLogProcessamento` para auditoria do dashboard.
  - `internal/state/fsm.go`: Correção do bug de formatação decimal (`%.2f` -> `%g`) na mensagem de sucesso.
  - `internal/state/fsm.go`: Orquestração da gravação automática de logs para todas as interações (registros, dúvidas, alertas).
  - Tabela `logs_processamento` criada no Supabase via migração automática (MCP).
