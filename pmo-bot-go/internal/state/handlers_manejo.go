...
func handleAguardandoQuantidade(ctx context.Context, body string, from string, phone string, profile *supabase.Profile, respondWithAudio bool, sbClient ports.DatabaseRepository, wpClient ports.ChannelSender, ttsClient ports.Synthesizer, historyManager *history.Manager, extraction map[string]interface{}, startTime time.Time, modelConfigured string) (string, ProcessResult) {
...
func finalizeRegistration(ctx context.Context, ext *groq.ExtractionResult, profile *supabase.Profile, sbClient ports.DatabaseRepository, _ ports.ChannelSender, _ ports.Synthesizer, _ string, originalBody string, _ bool, startTime time.Time, historyManager *history.Manager, phone string, modelConfigured string) (string, ProcessResult) {
