...
// SendPhoneOTP dispara um código de 6 dígitos por SMS para o telefone informado.
//
// Mesmo endpoint do SendEmailOTP (/auth/v1/otp), só troca o campo do payload —
// o GoTrue do Supabase decide sozinho, a partir do provider de SMS configurado
// no dashboard do projeto (Twilio, Vonage, MessageBird, Twilio Verify), como
// entregar. Sem um provider configurado, esta chamada retorna erro — por isso
// o chamador deve checar SMSOTPEnabled() antes de oferecer esta opção ao
// produtor, nunca prometer um caminho que ainda não está configurado.
//
// Pensado para o mercado moçambicano (baixo acesso a e-mail/internet, alta
// familiaridade com SMS/USSD tipo *155#): existe como alternativa ao vínculo
// por e-mail (SendEmailOTP) para quem tem conta mas não tem e-mail à mão.
func (c *Client) SendPhoneOTP(phone string) error {
	phone = strings.TrimSpace(phone)
	if phone == "" {
		return fmt.Errorf("SendPhoneOTP: telefone vazio")
	}
	payload := map[string]interface{}{
		"phone":       phone,
		"create_user": false, // Não cria conta nova se o número não bater com nenhuma existente
	}
	corpo, err := json.Marshal(payload)
	if err != nil {
		return fmt.Errorf("SendPhoneOTP: marshal: %w", err)
	}

	reqURL := fmt.Sprintf("%s/auth/v1/otp", c.config.URL)
	req, err := http.NewRequest(http.MethodPost, reqURL, strings.NewReader(string(corpo)))
	if err != nil {
		return fmt.Errorf("SendPhoneOTP: request: %w", err)
	}
	req.Header.Set("apikey", c.config.Key)
	req.Header.Set("Content-Type", "application/json")
	...
	resp, err := c.httpClient.Do(req)
	if err != nil {
		return fmt.Errorf("SendPhoneOTP: HTTP: %w", err)
	}
	defer resp.Body.Close()

	if resp.StatusCode >= 400 {
		respBody, _ := io.ReadAll(resp.Body)
		return fmt.Errorf("SendPhoneOTP: supabase (%d): %s", resp.StatusCode, string(respBody))
	}
	return nil
}

// VerifyPhoneOTP valida o código recebido por SMS e retorna o AuthUser.
func (c *Client) VerifyPhoneOTP(phone, token string) (*AuthUser, error) {
	payload := map[string]interface{}{
		"type":  "sms",
		"phone": phone,
		"token": token,
	...
	corpo, err := json.Marshal(payload)
	if err != nil {
		return nil, fmt.Errorf("VerifyPhoneOTP: marshal: %w", err)
	}

	reqURL := fmt.Sprintf("%s/auth/v1/verify", c.config.URL)
	req, err := http.NewRequest(http.MethodPost, reqURL, strings.NewReader(string(corpo)))
	if err != nil {
		return nil, fmt.Errorf("VerifyPhoneOTP: request: %w", err)
	}
	req.Header.Set("apikey", c.config.Key)
	req.Header.Set("Content-Type", "application/json")
	...
	resp, err := c.httpClient.Do(req)
	if err != nil {
		return nil, fmt.Errorf("VerifyPhoneOTP: HTTP: %w", err)
	}
	defer resp.Body.Close()

	respBody, _ := io.ReadAll(resp.Body)
	if resp.StatusCode >= 400 {
		return nil, fmt.Errorf("VerifyPhoneOTP: supabase (%d): %s", resp.StatusCode, string(respBody))
	}

	var res VerifyEmailOTPResult // mesmo formato de resposta do GoTrue, independente do canal
	if err := json.Unmarshal(respBody, &res); err != nil {
		return nil, fmt.Errorf("VerifyPhoneOTP: decode: %w", err)
	}
	if res.User.ID == "" {
		return nil, fmt.Errorf("VerifyPhoneOTP: supabase não retornou ID de usuário")
	}

	return &AuthUser{ID: res.User.ID, Phone: phone}, nil
}
...
// AttachUnconfirmedPhone anexa um telefone a um usuário existente SEM
// confirmar (phone_confirm: false) — diferente de LinkPhoneToUser, que marca
// como confirmado direto porque já rodou depois de uma verificação por
// e-mail. Este é o passo intermediário do vínculo por SMS: o GoTrue só aceita
// enviar OTP por telefone (SendPhoneOTP) para um número já associado a
// exatamente um usuário; sem isso, a chamada falharia com "user not found"
// mesmo que o telefone já esteja em profiles.telefone (uma coluna de dados
// comum, não a identidade de auth do GoTrue). VerifyPhoneOTP confirma este
// telefone de fato quando o código digitado bate.
func (c *Client) AttachUnconfirmedPhone(userID, phone string) error {
	payload := map[string]interface{}{
		"phone":         phone,
		"phone_confirm": false,
	}
	corpo, err := json.Marshal(payload)
	if err != nil {
		return fmt.Errorf("AttachUnconfirmedPhone: marshal: %w", err)
	}

	reqURL := fmt.Sprintf("%s/auth/v1/admin/users/%s", c.config.URL, userID)
	req, err := http.NewRequest(http.MethodPut, reqURL, strings.NewReader(string(corpo)))
	if err != nil {
		return fmt.Errorf("AttachUnconfirmedPhone: request: %w", err)
	}
	req.Header.Set("apikey", c.config.Key)
	req.Header.Set("Authorization", "Bearer "+c.config.Key)
	req.Header.Set("Content-Type", "application/json")
	...
	resp, err := c.httpClient.Do(req)
	if err != nil {
		return fmt.Errorf("AttachUnconfirmedPhone: HTTP: %w", err)
	}
	defer resp.Body.Close()

	if resp.StatusCode >= 400 {
		respBody, _ := io.ReadAll(resp.Body)
		return fmt.Errorf("AttachUnconfirmedPhone: supabase (%d): %s", resp.StatusCode, string(respBody))
	}

	log.Printf("🔗 [Auth] Telefone %s anexado (não confirmado) ao usuário %s, aguardando OTP", phone, userID)
	return nil
}
