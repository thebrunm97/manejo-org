...
_(nenhum item aberto no momento — ver 🟢 Concluído: F13, F17, F22 e F24)_
...
- **PMO e ZARC são amarrados à regulação brasileira.** `ZARC` (`internal/zarc`, `wiki/concepts/zarc-janela-de-plantio.md`) é dataset do MAPA (Ministério da Agricultura do Brasil), citado nas respostas do bot porque lastreia crédito rural e seguro agrícola brasileiros (Proagro) — não existe equivalente moçambicano no código, e provavelmente não existe o mesmo instrumento de crédito/seguro lá. O próprio conceito de "PMO" (Plano de Manejo Orgânico) é um documento de certificação orgânica do sistema brasileiro. **Resposta obtida em 2026-09-19 (ver [[certificacao-organica-mocambique]] no wiki):** Moçambique **não tem marco legal nacional** de agricultura orgânica ainda — existe certificação por **terceira parte** (organismos internacionais), e o governo está **desenvolvendo** legislação orgânica interna. Ou seja, não dá pra replicar o modelo brasileiro (documento com peso perante uma lei nacional específica) porque essa lei não existe lá. **Pergunta que ainda falta responder:** o produto mira no padrão de certificação por terceira parte (documentar o que organismos internacionais pedem, não uma lei nacional) já agora, ou espera a legislação moçambicana amadurecer? Isso não trava o resto do levantamento técnico (moeda, timezone, SMS/USSD já avançaram sem depender disso), mas trava o desenho específico do módulo de compliance/PMO pra lá.

**Gaps técnicos confirmados, sem decisão de negócio pendente — dá pra planejar:**
- ~~**Moeda hardcoded em R$.**~~ **Resolvido em 2026-09-18** — símbolo extraído para `utils.MoedaSimbolo()` (Go, `CURRENCY_SYMBOL` env) e `CURRENCY_SYMBOL`/`formatCurrency` (frontend, `VITE_CURRENCY_SYMBOL`), padrão `R$` preservado (zero mudança de comportamento hoje). Todos os 6 arquivos Go e 8 componentes de frontend que tinham `"R$"` literal agora leem do símbolo configurável — trocar de moeda em produção é só a env var. **Ainda falta, e isso sim é decisão de produto:** moeda POR conta/propriedade (hoje é uma única env global pra toda a instância) e o valor numérico de `DefaultLimiteTransacao` (50.000,00) em `business.go`, que é política de negócio, não símbolo — 50k BRL e 50k MZN não valem o mesmo. `go build/vet/test` e `tsc`/`vitest`/`npm run build` confirmados limpos.
...
- ~~**Timezone.**~~ **Auditado e corrigido em 2026-09-18** — achados 3 bugs reais, não só hipóteses: (1) `internal/weather/client.go` mandava `timezone=America/Sao_Paulo` fixo pra API da Open-Meteo **pra qualquer propriedade**, incluindo fora do fuso de São Paulo dentro do próprio Brasil (Manaus, Fernando de Noronha) — trocado pra `timezone=auto`, que a API resolve sozinha a partir de lat/lng, correto pra qualquer país. (2) `internal/prompt/manager.go` injetava "data atual" no prompt principal do LLM com fuso São Paulo hardcoded, mas `RouterSystemPrompt()` (prompt de classificação de intent) usava `time.Now()` cru — e o container não seta `TZ` (roda em UTC por padrão), então as duas funções podiam divergir sobre que dia era **mesmo hoje, sem sair do Brasil**. Unificado nas duas via `utils.TimezoneSistema()` (env `SYSTEM_TIMEZONE`, default `America/Sao_Paulo` — zero mudança de comportamento). **Ainda é uma env global pro deployment inteiro, não por propriedade** — isso exigiria saber o fuso de cada propriedade a partir de lat/lng ou país, decisão de modelagem pra quando o produto precisar de multi-fuso de verdade (ex.: Brasil e Moçambique atendidos pela mesma instância ao mesmo tempo). `go build/vet/test` confirmados limpos.
- **Consumo de dados — instrumentado em 2026-09-19, ainda sem dado real coletado.** Mapas de satélite (Mapbox/Google Tiles em `FarmMap`) pesam no plano de dados; o modo texto (DT-29) e o sync offline já ajudam aqui, mas o mapa em si não foi pensado pra isso. Em vez de mudar zoom/qualidade de tile às cegas (arriscaria piorar a experiência de todo mundo sem dado que justifique), adicionado `useMapDataUsageTracking` (`pmo-frontend/src/hooks/map/useMapDataUsageTracking.ts`), que mede via `PerformanceObserver` todo o tráfego de rede enquanto o mapa está aberto (tiles base, satélite, qualquer fonte — não precisa conhecer os hosts de antemão) e reporta pro Sentry (`map_data_usage`, com total em KB e detalhamento por host) ao desmontar, ao fechar a aba (`pagehide`) e a cada 2 minutos como rede de segurança em sessões longas. Zero mudança de comportamento do mapa — só mede. **Falta:** esperar acumular uso real em produção e então consultar o Sentry pra ter o número de verdade antes de decidir qualquer otimização. `tsc`/`vitest`/`npm run build` confirmados sem regressão (mesmo baseline do F28).
